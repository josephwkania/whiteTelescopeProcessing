...
class whiteTelescope(object):
    ...
    def takeData(self):
        ...
        if not os.path.exists(self.options["folderGiant"]):
            os.makedirs(self.options["folderGiant"])
...
if __name__ == "__main__":
    ...
    semi_opt.add_argument(
        "--waitTime",
        type=float,
        help="wait time inbetween frequency switches",
        default=10.0,
    )
    semi_opt.add_argument(
        "--integrationTime",
        type=float,
        help="integration time for spectra",
        default=280.0,
    ),
    semi_opt.add_argument(
        "--fast_integrationPulse",
        type=float,
        help="integration to use when making filterbanks",
        default=0.0005,
    )
    semi_opt.add_argument(
        "--freqOne",
        type=float,
        help="first frequency for frequency switiching",
        default=1.4207e9,
    )
    semi_opt.add_argument(
        "--freqTwo",
        type=float,
        help="second frequency for frequency switiching",
        default=1.4200e9,
    )
    semi_opt.add_argument(
        "--sampleRate", type=float, help="Rate which the USRP samples", default=40.0e6
    )
    semi_opt.add_argument("--vecLength", type=int, help="Length of FFT", default=1024)
    semi_opt.add_argument(
        "--decimationFactor",
        type=int,
        help="Factor to reduce spectral resolution for pulse mode",
        default=4,
    )
    semi_opt.add_argument(
        "--dirRoot",
        ...
        default="{0}/scratch".format(os.path.expanduser("~")),
    )

    # -----Below this line will be change to requard arguments in future versions-----#
    semi_opt.add_argument("--source", type=str, help="name of source", default="crab")
    ...
    semi_opt.add_argument(
        "--telescopeAZ", type=float, help="Telescope azimuth", default=180.0
    )
    semi_opt.add_argument(
        "--telescopeALT", type=float, help="Telescope altitude", default=72.0
    )
    semi_opt.add_argument(
        "--targetRA", type=str, help="Target's Right Ascension", default="05h43m32s"
    )
    semi_opt.add_argument(
        "--targetDEC", type=str, help="Target's Declination", default="22d00m52s"
    )
