...
os.chdir(
    "/home/odroid/gnuradio/lib/uhd/utils/share/uhd/images"
...
if separationNow < separationLimit:
    ...
    tb1 = headless_usrp_giantpulse.headless_usrp_giantpulse(
        fast_integration=fast_integrationPulse,
        prefix=prefixOne,
        samp_rate=sampleRate,
        vec_length=vecLength,
        freq=freqOne,
        giant_prefix=prefixGiant,
    ...
    spdic["nifs"] = int(1)
    spdic["fch1"] = float(
        (freqOne - sampleRate / 2.0 + sampleRate / (vecLength / decimationFactor))
        / 10.0 ** 6.0
    )
    ...
    tb1 = headless_usrp.headless_usrp(
        fast_integration=fast_integrationNoPulse,
        prefix=prefixOne,
        samp_rate=sampleRate,
        vec_length=vecLength,
        freq=freqOne,
...
if separationNow < separationLimit:
    ...
    mover2 = Popen(
        ...
        + tb2.giantout_bin.split(".bin")[0]
        + ".fil "
        + "bowser:/hyrule/data/users/jwkania/odroid/giantPulses/",
        stdout=PIPE,
        shell=True,
    )
...
movie = Popen(
    "/bin/bash /home/odroid/Documents/dspira-master/grc-flowgraphs/movieMaker.sh",
    stdout=PIPE,
    shell=True,
)
