...
if separationNow < separationLimit:
    ...
    tb1 = headless_usrp_giantpulse.headless_usrp_giantpulse(fast_integration=fast_integrationPulse, prefix=prefixOne, samp_rate=sampleRate, vec_length=vecLength, freq=freqOne, giant_prefix=prefixGiant, decimation_factor=decimationFactor)#this is copied from gnuradio's main()
    tb1.start()
    ...
    spdic["nbits"] = int(32)
    spdic["nchans"] = int(vecLength/decimationFactor)
    spdic["nifs"] = int(1)
    spdic["fch1"] = float((freqOne-sampleRate/2.0+sampleRate/(vecLength/decimationFactor))/10.0**6.0)
    spdic["foff"] = float(sampleRate/(vecLength/decimationFactor*10.0**6.0))
...
if separationNow < separationLimit:
    tb2 =  headless_usrp_giantpulse.headless_usrp_giantpulse(fast_integration=fast_integrationPulse, prefix=prefixTwo, samp_rate=sampleRate, vec_length=vecLength, freq=freqTwo, giant_prefix=prefixGiant, decimation_factor=decimationFactor)
    tb2.start()
    time.sleep(integrationTime)
    tb2.stop()
    tb2.wait()
    ...
    mover2 = Popen('rsync -ah --remove-source-files '+tb2.giantout_bin.split('.bin')[0]+".fil " + 'bowser:/hyrule/data/users/jwkania/odroid/giantPulses/',stdout=PIPE, shell=True)
    ...
    #remover.wait()
    mover = Popen('rsync -ah --remove-source-files '+tb1.giantout_bin.split('.bin')[0]+".fil " + ' bowser:/hyrule/data/users/jwkania/odroid/giantPulses/', stdout=PIPE, shell=True)
