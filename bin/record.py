...
class whiteTelescope(object):
    ...
    def takeData(self):
        ...
        self.options["DECJ"]= pointingRADEC.dec.dms[0]*10000 + pointingRADEC.dec.dms[1]*100 + pointingRADEC.dec.dms[2]
        self.options["RAJ"]= pointingRADEC.ra.hms[0]*10000 + pointingRADEC.ra.hms[1]*100 + pointingRADEC.ra.dms[2]
        self.options["telescopeRA"] = pointingRADEC.ra.to_string(u.hour)
        self.options["telescopeDEC"] = pointingRADEC.dec.to_string(u.degree, alwayssign=True)
        separationNow = target.separation(pointingRADEC).degree
        print("Telescope-{0} Separation = {1:.2f} deg".format(self.options["source"],separationNow))
...
if __name__ == "__main__":
    ...
    semi_opt.add_argument('--freqOne', type=float,
                           help="first frequency for frequency switiching", default=1.4207e9)
