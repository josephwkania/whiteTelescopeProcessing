...
class bandPassPlotter(object):
    ...
    def movieMaker(self):
        ...
        imageio.mimsave(
            self.options["folderImage"] + self.options["UTC"] + ".gif",
            imageList,
            "GIF",
            **kargs
        )
    ...
    def plot(self):
        try:
            ...
            spectrum = f["spectrum"][:].mean(
                axis=0
            )  # get the spectrum data, mean flattens the data
            fileOne = True
        except:
            fileOne = False
        ...
        if not fileOne and not fileTwo:
            ...
            not fileOne and fileTwo
        ):  # check to make sure first spectrum is there, it it isn't, plot the second file
            print("First file not present, plotting second")
            spectrum = spectrum2
            self.switch = False  # if only both file are present do freq switching
            self.options["freqOne"] = self.options[
                "freqTwo"
            ]  # makes sure csv has correct freq
        ...
        def fitter(x, a, b, c, m, o):  # for curve fitting
            ...
            return (
                a * np.exp(-((x - b) ** 2.0) / c ** 2.0)
                - a * np.exp(-((x - b - offset) ** 2.0) / c ** 2.0)
                + m * x
                + o
            )
        ...
        # print fstep
        freq = (
            np.arange(self.options["vecLength"]) * fstep + fstart
        ) / 10.0 ** 6.0  # make an array of x-axis indices
        ...
        ax1.set_xlabel(
            "frequency [MHz] (w/ cntr={0}MHz)".format(
                self.options["freqOne"] / 10.0 ** 6.0
            )
        )
        ax1.get_xaxis().get_major_formatter().set_useOffset(
            False
        )  # makes plt print out full axis
        ax2.set_xlabel(
            "km/s (w/ cntr={0}MHz)".format(self.options["freqOne"] / 10.0 ** 6.0)
        )
        ax1.set_ylabel("Counts")
        # new_tick_locations = np.array([1420.0, 1420.25, 1420.4, 1420.5, 1420.75, 1421.0])
        new_tick_locations = np.array(
            [1410.0, 1415.0, 1420.0, 1420.4, 1425.0, 1430.0, 1435.0]
        )
        ax2.set_xticks(new_tick_locations)
        ax2.set_xticklabels(tick_function(new_tick_locations))
        plt.title(
            "{0}, l={1:.1f}, b={2:.1f}".format(
                self.options["fileOne"].split("/")[-1],
                self.options["pointingGalacticL"],
                self.options["pointingGalacticB"],
            ),
            y=0.93,
        )
        ...
        if self.switch:
            ...
            plt.plot(
                freq,
                spectrum,
                label="Cntr{0:.1f}MHz".format(self.options["freqOne"] / 10.0 ** 6.0),
            )  # divide by 10e6 to conver Hz to MHz
            plt.plot(
                freq,
                spectrum2,
                label="Cntr{0:.1f}MHz".format(self.options["freqTwo"] / 10.0 ** 6.0),
            )
            plt.plot(
                freq,
                diff,
                label="{0:.1f}-{1:.1f}".format(
                    self.options["freqTwo"] / 10.0 ** 6.0,
                    self.options["freqOne"] / 10.0 ** 6.0,
                ),
            )
            ...
            cleanedFlux = np.sum(trimmedSpectrum) - np.sum(
                trimmedDiff[startIndex:endIndex]
            )
            print("Cleaned Integrated Flux = {0:.1f}".format(cleanedFlux))

            plt.plot(trimmedFreq, fitter(trimmedFreq, *popt))

            self.options["continuumFile"] = "{0}continuum.csv".format(
                self.options["folderCSV"]
            )
            with open(self.options["continuumFile"], "a") as csvContinuum:
                ...
                csvContinuumWriter.writerow(
                    [
                        self.options["fileOne"].split("/")[-1].split("_Drift.h5")[0],
                        self.options["RAinHour"],
                        self.options["DECinDeg"],
            ...
            with open(
                "{0}{1}.csv".format(self.options["folderCSV"], self.options["timeNow"]),
                "w",
            ) as csvfile:  # writes files to csv
                csvwriter = csv.writer(csvfile, dialect="excel")
                csvfile.write(
                    "#az={0},alt={1},start-freqOne={2}UTC,start-freqTwo={3}\n".format(
                        self.options["telescopeAZ"],
                        self.options["telescopeALT"],
                        self.options["fileOne"].split("/")[-1].split("_D")[0],
                        self.options["fileTwo"].split("/")[-1].split("_D")[0],
                    )
                )
                csvfile.write(
                    "#freqOne={0},freqTwo={1},sampleRate={2},numberOfFFTs={3}\n".format(
                ...
                csvfile.write(
                    "#source={0},integrationTime={1},waitTime={2}".format(
                        self.options["source"],
                        self.options["integrationTime"],
            ...
            plt.plot(
                10.0 * np.log10(spectrum.mean(axis=0)),
                label="Cntr{0:.1f}MHz".format(self.options["freqOne"] / 10.0 ** 6.0),
            )
            ...
            with open(
                "{0}{1}.csv".format(
                    self.options["folderCSV"],
                    self.options["primarySpec.split"].split(".h5")[0],
                ).split("/")[-1],
                "w",
            ) as csvfile:  # writes files to csv
                csvwriter = csv.writer(csvfile, dialect="excel")
                csvfile.write(
                    "#az={0},alt={1},start-freqOne={2}UTC\n".format(
                        self.options["telescopeAZ"],
                        self.options["telescopeALT"],
                        self.options["fileOne"].split("/")[-1].split("_D")[0],
                    )
                ...
                csvfile.write(
                    "#freqOne={0},sampleRate={1},numberOfFFTs={2}\n".format(
                        self.options["freqOne"],
                        self.options["sampleRate"],
                        self.options["vecLength"],
                    )
        ...
        plt.savefig(
            self.options["folderImage"]
            + self.options["fileOne"].split("h5")[0].split("/")[-1]
            + "png",
            dpi=110,
...
if __name__ == "__main__":  # if the program is run at top level
    ...
    required.add_argument("--fileOne", type=str, help="first .h5 file", required=True)
    required.add_argument(
        "--folderImage", type=str, help="location of .png files", required=True
    )

    semi_opt = parser.add_argument_group("arguments set to defaults:")
    semi_opt.add_argument(
        "--telescopeAZ", type=float, help="Telescope pointing Azimuth", default=0.0
    )
    semi_opt.add_argument(
        "--telescopeALT", type=float, help="Telescope pointing Altitiude", default=0.0
    )
    semi_opt.add_argument(
        "--source", type=str, help="name of source telescope is looking at.", default=""
    )
    semi_opt.add_argument(
        "--integrationTime", type=float, help="time data gathered", default=0.0
    )
    semi_opt.add_argument(
        "--sampleRate", type=float, help="Sample rate of USRP", default=3.0e6
    )
    semi_opt.add_argument(
        "--freqOne",
        type=float,
        help="Center frequency of first data set",
        default=1420.7e6,
    )
    semi_opt.add_argument(
        "--freqTwo",
        type=float,
        help="Center frequency of second data set",
        default=1420.0e6,
    )
    semi_opt.add_argument(
        "--pointingGalacticL",
        type=float,
        help="glactic l coordinate of souce [in degrees]",
        default=0.0,
    )
    semi_opt.add_argument(
        "--pointingGalacticB",
        type=float,
        help="galactic b coordinate of source [in degrees]",
        default=0.0,
    )
    optional = parser.add_argument_group("other optional arguments:")
    optional.add_argument(
        "--fileTwo", type=str, nargs="+", help="Second file to do frequency switching"
    )
